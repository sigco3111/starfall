...
| 영역 | 상태 |
|---|---|
| 부팅 화면 (boot) | ✅ 정적 한국어 + data-i18n fallback |
| 상단 HUD (자원·보급·함대·작전·경과) | ✅ `resourceUnits` / `fleetSupply` / `fleet` / `ops` / `elapsed` |
| Ops 알림 (피격 중·수집기 대기·건조 중) | ✅ `opsUnderFire` / `opsCollectorIdle` / `opsInBuild` |
| 명령 바 (이동·공격·정지·채집·귀환) | ✅ `cmdMoveLabel` … `cmdDockLabel` + `cmdMoveBody` … `cmdDockBody` |
| 편대 6종 + 스탠스 4종 | ✅ `formationDelta` … `formationLine` + `stanceAggressive` … `stanceEvasive` |
| 13종 함선 클래스 표시 이름 | ✅ `shipDisplayName()` (gameplay 식별자 보존 + UI 한국어 분리) |
| 생산 패널 (생산·연구·집결지·연구 트리 9종 + description) | ✅ `production` / `buildQueue` / `research` / `researchDesc*` |
| 함대 명단 바 (IDLE 칩) | ✅ `fleetIdle` |
| 센서 매니저 미니맵 배너 | ✅ `sensorsManager` |
| 메뉴 (조작·크레딧·오디오·마스터/효과음/음악) | ✅ `menuTabControls` / `menuTabCredits` / `menuTabAudio` / `menuAudio*` |
| 메뉴 > 조작 탭 (CONTROL_HELP) | ✅ `ctrlGroupSelecting` / `ctrlGroupOrdering` / `ctrlGroupCamera` + 키 액션 9종 |
| 인게임 크레딧 (Homeworld 헌정·Claude Opus 5·OpenGameArt·Kenney) | ✅ 한국어 (브랜드·저작자명 보존) |
| SFX 팩 description 3종 | ✅ `sfxPackUseSciFi` / `sfxPackUseImpact` / `sfxPackUseInterface` |
| 인게임 notice 메시지 5건 | ✅ `noticeMoveConfirmed` / `noticeAttacking` / `noticeHarvesting` / `noticeAttackMove` / `noticeHoldingPosition` |
| 노스크립트 fallback (검색엔진·JS OFF 사용자용) | ✅ 정적 한국어 HTML |
| `<title>`, `<meta description>`, OpenGraph, Twitter Card | ✅ 한국어 + 정직한 GitHub Pages URL |
| GitHub Pages 배포 (`/starfall/`) | ✅ 정적 SPA |

**i18n 키 168개 / EN ↔ KO 100% 매칭**, 한자 0건, `tsc --noEmit` 0 에러.

**보존된 것**: 13종 함선 코드네임(Probe Scout/Talon Interceptor/...), 브랜드(Homeworld, Claude Opus 5, Mike Luan, Vite, TypeScript, Three.js, e01.ai), 라이선스(MIT), 엔진 동작, 시뮬레이션 로직, 음악 트랙 제목(저작자 표기 표준), 키 라벨 자체(`M`/`S`/`H`/`A`/`D`).
...
- **i18n 모듈**: `src/i18n.ts` (KO/EN 평탄 dict 168 키 + `t()` + `setLanguage()` + `shipDisplayName()` + `applyStatic()`)
...
**이 fork의 추가분**: 한국어 번역 + GitHub Pages 배포.
...
1. **번역 데이터**: `src/i18n.ts`에 한국어 dict 168 키 추가 + 모든 사용자 가시 리터럴을 `t()` 호출로 감쌈
2. **부팅 화면**: `index.html`의 `lang="en"` → `"ko"`, 메타 설명·타이틀·OpenGraph 한국어화, 부팅 텍스트 `data-i18n` 속성 + 런타임 `applyStatic()` fallback
...
빌드는 `tsc --noEmit && vite build`로 1초 이내 통과하며 0 에러, 한자 0건.

## 상태 뱃지

![Live](https://img.shields.io/badge/Live-sigco3111.github.io%2Fstarfall-blue)
![License](https://img.shields.io/badge/License-MIT-green)
![Upstream](https://img.shields.io/badge/Upstream-e01--ai%2Fstarfall-orange)
![Korean fork](https://img.shields.io/badge/i18n-KO%2FEN%20168-yellow)